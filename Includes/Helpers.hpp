...
namespace CTRPluginFramework {
    ...
    enum class GameName {None, Sun, Moon, US, UM};

    // Enumeration to categorize game sets (Base vs Ultra)
    enum class GameSet {None, Base, Ultra};

    // External variables to hold the current game ID and set
    extern GameName currGameName;
    extern GameSet currGameSet;

    // Returns baseSet if currGameSet is Base; otherwise, returns ultraSet
    template<typename T>
    T AutoGameSet(T baseSet, T ultraSet) {
        return (currGameSet == GameSet::Base) ? baseSet : ultraSet;
    }
    ...
    // Function to return an address based on whether the game ID is Sun/US or Moon/UM
    u32 AutoGame(u32 addrA, u32 addrB);

    // Function to detect the game and set based on title ID and version
    void DetectGame(u32 titleID);

    // If the update is supported or not
    bool IsUpdateSupported(u16 version);

    // External vector for battle offsets
    extern vector<u32> battleOffset;

    // Configures battle offsets
    void SetBattleOffset(void);

    // Checks if in a battle
    bool IfInBattle(void);

    // List of all the Pokemon
    static const char *speciesList[807] = {
        "Bulbasaur",
        "Ivysaur",
        "Venusaur",
        "Charmander",
    ...
    static const vector<string> formList(int dex) {
        ...
        switch (dex) {
            ...
            case 25:  // Pikachu
                options = AutoGameSet({"Normal"}, {"Normal", "Original Cap", "Hoenn Cap", "Sinnoh Cap", "Unova Cap", "Kalos Cap", "Alola Cap", "Partner Cap"});
                break;
            ...
            case 800: // Necrozma
                options = AutoGameSet({"Normal"}, {"Normal", "Dawn", "Dusk", "Ultra"});
                break;
    ...
    static const char *abilityList[233] = {
        "Stench",
        "Drizzle",
        "Speed Boost",
        "Battle Armor",
    ...
    static const char *heldItemList[959] = {
        "Master Ball",
        "Ultra Ball",
        "Great Ball",
        "Poké Ball",
    ...
    static const char *movesList[728] = {
        "Pound",
        "Karate Chop",
        "Double Slap",
        "Comet Punch",
    ...
    static const Balls ballList[26] = {
        {4, "Poké Ball", 3},
        {3, "Great Ball", 3},
        {2, "Ultra Ball", 3},
        {26, "Beast Ball", 7},
