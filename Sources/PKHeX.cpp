...
namespace CTRPluginFramework {
    ...
    static void UpdateIDValue(const vector<u32> &addresses, const vector<string> &options) {
        ...
        if (selectedID >= 0) {
            ...
            if (KeyboardHandler<u32>::Set(options[selectedID] + ":", true, false, 5, newIDValues[selectedID], 0, 1, 65535, nullptr)) {
                Process::Write16(addresses[selectedID], newIDValues[selectedID]);
                OSD::Notify("Success!");
            }
        }
    }
    ...
    void Identity(MenuEntry *entry) {
        static const vector<u32> addresses = {AutoGameSet({0x330D67D0, 0x33012818}, {0x330D67D2, 0x3301281A})};
    ...
    void InGameName(MenuEntry *entry) {
        static const u32 address = AutoGameSet(0x330D6808, 0x33012850);
    ...
    void UpdatePlayTime(MenuEntry *entry) {
        static const u32 address = AutoGameSet(0x34197648, 0x33F8127C);
    ...
    void UpdateLanguage(MenuEntry *entry) {
        static const u32 address = AutoGameSet(0x330D6805, 0x3301284D);
        u8 currentLanguage;
    ...
    void ApplyMoney(MenuEntry *entry) {
        static const u32 address = AutoGameSet(0x330D8FC0, 0x33015210);
        u32 currentMoney;
    ...
    void ApplyBattlePoints(MenuEntry *entry) {
        static const u32 address = AutoGameSet(0x330D90D8, 0x33015328);
        u16 currentPoints;
    ...
    void UpdateItems(MenuEntry *entry) {
        static const vector<u32> address = {AutoGameSet({0x330D5934, 0x330D5936}, {0x33011934, 0x33011936})};
        // Initialize itemList with existing items and additional items
        unordered_set<int> itemSet(items.begin(), items.end());
        // Add extra items
        vector<int> extraItems = {534, 535, 752, 753, 754, 755, 756, 757, 758, 759, 760, 761, 762, 763, 764, 767, 768, 769, 770, 111, 137, 795, 796, 844, 846, 849, 851, 853, 854, 855, 856, 879, 880, 881, 882, 883, 884, 904, 905, 906, 907, 908, 909, 910, 911, 912, 913, 914, 915, 916, 917, 918, 919, 920};
        // Insert extra items into itemSet
        itemSet.insert(extraItems.begin(), extraItems.end());
        // Convert unordered_set back to vector and sort it
        vector<int> itemList(itemSet.begin(), itemSet.end());
        sort(itemList.begin(), itemList.end());

        for (int i = 0; i < itemList.size(); ++i)
            Process::Write32(address[0] + (0x4 * i), itemList[i] + (itemAmount << 10));
    }
    ...
    void UpdateMedicines(MenuEntry *entry) {
        static const vector<u32> address = {AutoGameSet({0x330D647C, 0x330D647E}, {0x330124A8, 0x330124AA})};
        unordered_set<int> medicineSet(medicines.begin(), medicines.end()); // Initialize with existing medicines
        // Add extra medicines
        medicineSet.insert({852, 65, 66, 67});
        // Convert unordered_set back to vector and sort it
        vector<int> medicineList(medicineSet.begin(), medicineSet.end());
        sort(medicineList.begin(), medicineList.end());

        for (int i = 0; i < medicineList.size(); ++i)
            Process::Write32(address[0] + (0x4 * i), medicineList[i] + (medAmount << 10));
    }
    ...
    void UpdateBerries(MenuEntry *entry) {
        static const vector<u32> address = {AutoGameSet({0x330D657C, 0x330D657E}, {0x33012598, 0x3301259A})};
        unordered_set<int> berrySet(berries.begin(), berries.end()); // Initialize with existing berries
        // Convert unordered_set back to vector and sort it
        vector<int> berryList(berrySet.begin(), berrySet.end());
        sort(berryList.begin(), berryList.end());

        for (int i = 0; i < berryList.size(); ++i)
            Process::Write32(address[0] + (0x4 * i), berryList[i] + (berryAmount << 10));
    }
    ...
    void UnlockTMsAndHMs(void) {
        static const vector<u32> address = {AutoGameSet({0x330D62CC, 0x330D62CE}, {0x330122F8, 0x330122FA})};
        unordered_set<int> teachableSet(teachables.begin(), teachables.end()); // Initialize with existing teachables
        // Convert unordered_set back to vector and sort it
        vector<int> teachableList(teachableSet.begin(), teachableSet.end());
        sort(teachableList.begin(), teachableList.end());

        // Unlock TMs & HMs
        for (int i = 0; i < teachableList.size(); ++i)
            Process::Write32(address[0] + (0x4 * i), teachableList[i] + (1 << 10));

        OSD::Notify("Unlocked all TMs & HMs!");
    }

    void UnlockKeyItems(void) {
        static const vector<u32> address = {AutoGameSet({0x330D5FEC, 0x330D5FEE}, {0x33011FE0, 0x33011FE2})};
        unordered_set<int> keyItemSet = {216, 465, 466, 628, 629, 631, 632, 638, 431, 442, 450, 641, 642, 643, 689, 695, 696, 698, 700, 701, 702, 703, 705, 712, 713, 714, 705, 706, 765, 773, 797, 841, 842, 843, 845, 847, 850, 857, 858, 860};
        // Convert unordered_set back to vector and sort it
        vector<int> keyItemList(keyItemSet.begin(), keyItemSet.end());
        sort(keyItemList.begin(), keyItemList.end());

        // Unlock Key Items
        for (int i = 0; i < keyItemList.size(); ++i)
            Process::Write32(address[0] + (0x4 * i), keyItemList[i] + (1 << 10));
    ...
    bool ExportData(const string &directory, u32 address, const string &fileExtension) {
        ...
        if (KeyboardHandler<string>::Set("Name:", true, 16, fileName, "", nullptr)) {
            ...
            backupFile.Dump(address, 222720);
    ...
    bool ImportData(const string &directory, u32 address, const vector<string> &fileList) {
        ...
        if (fileIndex >= 0) {
            ...
            restoreFile.Inject(address, 222720);
    ...
    void ExportImport(MenuEntry *entry) {
        ...
        const u32 backupAddr = AutoGameSet(0x330D9838, 0x33015AB0);
    ...
    bool VerifyPokemonData(Pokemon data) {
        ...
        return (Checksum(data) == data->checksum) &&
           (data->species >= 1 && data->species <= AutoGameSet(802, 807));
    }
    ...
    namespace PKHeX {
        ...
        void AssignSpecies(Pokemon data, u16 getSpecies) {
            ...
            if (getSpecies > 0 && getSpecies <= AutoGameSet(802, 807) && data->species != getSpecies)
        ...
        u32 DetermineSpeciesPointer() {
            return AutoGameSet(0x330D9838, 0x33015AB0);
        }
        ...
        bool VerifyPokemonData(u32 pointer, PK7 *pokemon) {
        ...
        bool HandlePokemonData(PK7 &pokemon, MenuEntry *entry, const function<bool(PK7&)> &processFunc, const function<string()> &getMessage) {
        ...
        void Shiny(MenuEntry *entry) {
            ...
            HandlePokemonData(pokemon, entry, [&](PK7 &data) -> bool {
        ...
        void Level(MenuEntry *entry) {
            ...
            auto processLevel = [&](PK7 &data) -> bool {
    ...
    void BoxesUnlocked(MenuEntry *entry) {
        static const u32 address = AutoGameSet(0x330D982D, 0x33015AA5);

        // Lambda function to check if the boxes are unlocked
        auto isBoxesUnlocked = [](u8 value) -> bool {
            return value >= 32; // Check if the boxes are fully unlocked (value >= 32)
        };

        // Function to unlock the boxes
        auto unlockBoxes = [&]() -> bool {
            return Process::Write8(address, 32); // Write 32 to the address to unlock all boxes
        };
    ...
    void PCAnywhere(MenuEntry *entry) {
        static const vector<u32> address = {
            AutoGameSet({0x597878, 0x37E5E4, 0x374C3C, 0x5978A8},
                        {0x5BBEE0, 0x37DB28, 0x38B030, 0x5BBF10})
        };

        // Define the instruction set
        static const vector<u32> instructions = {
            0xE92D400E, 0xE59F1074, 0xE5912000, 0xE3A03000, 0xE5813000, 0xE3A03C13, 0xE2833093,
            0xE1520003, 0x03A04000, 0x11A04000, 0xE8BD800E, 0xE350000E,
            AutoGameSet<u32>(0xEAF77958, 0xEAF70DE3),
            0xE92D400F, 0xE24DD010, 0xE3A00000, 0xE58D0000,
            0xE58D0004, 0xE58D0008, 0xE58D000C, 0xE1A03000, 0xE1A02000,
            0xE3A01C13, 0xE2811093, 0xE59F0018, 0xE5801000,
            0xE59F0014, 0xE5900000,
            AutoGameSet<u32>(0xEBF7D6DC, 0xEBF77D15),
            0xE3A00002, 0xE28DD010, 0xE8BD800F,
            AutoGameSet<u32>(0x638000, 0x669000),
            AutoGameSet<u32>(0x6740BC, 0x6A59C4)
        };

        // Original instructions and revert data
        static vector<u32> original(instructions.size()), revert(3);
        static bool saved[2] = {false, false};

        // Helper lambda to write to process memory
        auto writeProcessMemory = [&](u32 dest, const vector<u32> &data, vector<u32> &backup, bool &saveFlag) -> bool {
            if (!Process::Write32(dest, data, backup, entry, saveFlag))
                return false;

            return true;
        };

        // Write new instructions to the first address
        if (writeProcessMemory(address[0], instructions, original, saved[0])) {
            // Write revert values to the remaining addresses
            vector<u32> revertInstructions = {
                AutoGameSet<u32>(0xEB0864A3, 0xEB08F8F7),
                AutoGameSet<u32>(0xEB088B18, 0xEB08C3AA),
                AutoGameSet<u32>(0x1AF77958, 0x1AF70DE3)
            };

            vector<u32> revertAddresses = {address[1], address[2], address[3]};

            if (!Process::Write32(revertAddresses, revertInstructions, revert, entry, saved[1]))
                return;  // Early exit if writing fails
        }
    }

    bool ClonePokemon(u8 srcBox, u8 srcPosition, u8 destBox, u8 destPosition) {
        PK7 pokemon; // Pokemon object to hold data
    ...
    bool ValidateBoxPosition(u8 box, u8 position) {
        return (box >= 1 && box <= 32) && (position >= 1 && position <= 32);
    }

    bool SetBoxPosition(u8 &srcBox, u8 &srcPosition, u8 &destBox, u8 &destPosition) {
        // Get source box and position from the user
        if (!KeyboardHandler<u8>::Set("Copy from which box:", true, false, 2, srcBox, 1, 1, 32) ||
            !KeyboardHandler<u8>::Set("Which position in box " + Utils::ToString(srcBox, 0) + ":", true, false, 2, srcPosition, 1, 1, 32)) {
            OSD::Notify("Failed to get source box or position");
            return false;
        }

        // Get destination box and position from the user
        if (!KeyboardHandler<u8>::Set("Paste to box:", true, false, 2, destBox, 1, 1, 32) ||
            !KeyboardHandler<u8>::Set("Which position in box " + Utils::ToString(destBox, 0) + " to paste:", true, false, 2, destPosition, 1, 1, 32)) {
